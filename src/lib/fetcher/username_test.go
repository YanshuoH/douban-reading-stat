...
const (
  ...
  URL_TEST_6 = "https://www.douban.com/people/test-123/"
  URL_TEST_7 = "https://www.douban.com/people/test-123--/"
)
...
func TestFetchUsername(t *testing.T) {
  ...
  username, err = FetchUsername(URL_TEST_6)
  assert.Nil(err, "Fetcher", "Should not be an error for int username")
  assert.Equal("test-123", username, "Fetcher", "Should be the user test-123")

  username, err = FetchUsername(URL_TEST_7)
  assert.Nil(err, "Fetcher", "Should not be an error for int username")
  assert.Equal("test-123--", username, "Fetcher", "Should be the user test-123--")
}
